...
Space::Space(SpaceSettings settings)
{
    m_Shader = std::make_shared<Shader>(std::initializer_list<ShaderElement> {
        {ShaderType::VERTEX, "Assets/Shaders/space.vert"},
        {ShaderType::FRAGMENT, "Assets/Shaders/space.frag"}});
    m_Shader->Build();
...
void Space::Draw(std::shared_ptr<OrbitalCamera> m_Camera)
{
    m_Shader->Bind();
...
void Space::GenerateSphere(int latitudeSegments,
